 ...
 * On GitHub Pages production builds, this is '/GGC_MAPS_TEST'
 * On development, this is ''
 */
export const getBasePath = () => {
  if (typeof window === 'undefined') {
    // Server-side rendering
    return process.env.NODE_ENV === 'production' ? '/GGC_MAPS_TEST' : '';
  }
...
export const processBuildingsData = (buildingsData) => {
  // Note: Do NOT apply assetPath() here. The file paths are used by components
  // (FloorMapView, InlineSvg) that already handle basePath internally via getAssetPath()
